...
- 3.6.0 bug fix (Discord: snowfall hateall): Custom fan curves now detect the v3.5.0 "accepted positive write, RPM stays at 0" failure mode and send a bounded one-shot wake pulse instead of requiring users to switch curves manually to wake the EC.
